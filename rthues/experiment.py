...
from visigoth.stimuli import Point, Points
from visigoth.stimuli.dots import RandomDotColorMotion
...
def create_stimuli(exp):
    ...
    dots = RandomDotColorMotion(exp.win,
...
def generate_trials(exp):
    ...
    for _ in exp.trial_count(max=exp.p.trials_per_run):
        ...
        target = flexible_values(range(len(exp.p.dot_hue)))
...
def run_trial(exp, info):
    ...
    for i in exp.frame_range(seconds=exp.p.wait_resp):
        ...
        exp.s.dots.update(0, 0, info["dot_hue"], info["dot_coh"])
